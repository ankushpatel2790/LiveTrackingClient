...
public class LwM2mNodeEncoder {
    ...
    public static byte[] encode(LwM2mNode node, ContentFormat format, LwM2mPath path, LwM2mModel model) {
        ...
        LOG.info("Encoding node {} for path {} and format {}", node, path, format);

        // if (format == ContentFormat.TLV
        // && (path.getObjectId() == 30001 || path.getObjectId() == 30002 || path.getObjectId() == 30003
        // || path.getObjectId() == 30004 || path.getObjectId() == 30005 || path.getObjectId() == 30006))
        ...
        LOG.debug("Encoded node {}: {}", node, Arrays.toString(encoded));
        return encoded;
    }
}
